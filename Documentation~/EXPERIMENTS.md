...
## Эксперимент 2026-08-06 — Ограничение внутренних размеров xatlas

- **Проблема:** публичные `RepackOptions` позволяли передать в xatlas
  переполненное или чрезмерно большое произведение `resolution × internalOversample`,
  что могло надолго заблокировать либо аварийно завершить Unity Editor.
- **Изменение:** единый preflight для single- и multi-mesh путей считает resolution
  и padding в `ulong`, ограничивает oversample значением 16, а внутреннюю сторону
  атласа значением 8192, а padding — 256 пикселями до любого вызова native plugin.
- **Совместимость:** штатные значения до 8192 включительно не изменяются; опасные
  конфигурации завершаются контролируемой ошибкой без изменения mesh.
- **Проверка:** EditMode-тесты покрывают превышение лимита, `uint`-overflow input
  и oversample выше поддерживаемого UI диапазона.
