...
## Эксперимент 2026-08-06 — ARAP только по явному запросу

- **Проблема:** автоматическая ARAP-репараметризация была включена по умолчанию и могла синхронно запускать дорогостоящий решатель для каждого растянутого UV-шела без общего лимита работы.
- **Изменение:** значения по умолчанию в `UvToolContext` и `RepackOptions` выключены; сама возможность ARAP, порог и число итераций сохранены и доступны по явному запросу.
- **Ожидание/проверка:** обычный repack не запускает ARAP, а пользователи и программные вызовы могут включить прежнее поведение явно.
