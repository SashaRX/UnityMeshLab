...
namespace LightmapUvTool
{
    public class Uv2AssetPostprocessor : AssetPostprocessor
    {
        ...
        /// <summary>
        /// Apply UV2 from sidecar to mesh by position+UV0 matching.
        /// No meshopt/weld — the FBX mesh keeps its original vertex layout.
        /// Sidecar may have fewer vertices (post-weld); multiple FBX vertices
        /// at the same position share the same sidecar UV2.
        /// </summary>
        static bool ApplyEntryToMesh(Uv2DataAsset data, Mesh mesh)
        {
            ...
            // No stored positions — legacy path, direct assign if counts match
            ...
            // Build sidecar position → indices lookup
            ...
            // UV0 for disambiguation at seams: sidecar UV0 was saved after
            // NormalizeShellWinding, so we normalize the FBX mesh UV0 too
            // (only for matching, not modifying the mesh).
            ...
            for (int i = 0; i < meshCount; i++)
            {
                ...
                {
                    // Multiple sidecar verts at same position — pick closest UV0
            ...
            // Fallback: nearest-neighbor for unmatched (quantization boundary)
        ...
        static float SqrDist3(Vector3 a, Vector3 b)
        {
            ...
            for (int i = 0; i < meshCount; i++)
            {
                ...
                {
                    // No UV0 — pick first candidate (all share same position)
                    result[i] = entry.uv2[candidates[0]];
                    matched++;
                }
            }
        ...
        /// <summary>Pack position into a single long for fast dictionary lookup (0.1mm precision).</summary>
        static long PackPos(Vector3 p)        {
            long x = Mathf.RoundToInt(p.x * 10000f);
            long y = Mathf.RoundToInt(p.y * 10000f);
            long z = Mathf.RoundToInt(p.z * 10000f);
