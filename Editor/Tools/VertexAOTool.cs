...
namespace SashaRX.UnityMeshLab
{
    public class VertexAOTool : IUvTool
    {
        ...
        // still bounds the total bake time and retained result arrays. The mesh-count cap
        // is deliberately generous: SameRootNearby occluder gathering legitimately
        // collects hundreds of small meshes around a target.
        ...
        const int MaxMeshesPerLodBatch = 2048;
        ...
        void ExecuteBake()
        {
            ...
            if (!TryValidateBakeWorkload(batches, out string workloadError))
            {
                ...
                UvtLog.Warn("[Vertex AO] Bake aborted — " + workloadError);
                return;
            }
        ...
        /// <summary>
        /// Pre-flight check run before any bake work starts. Refuses workloads whose
        /// size would exhaust editor/GPU memory instead of discovering it mid-bake.
        /// </summary>
            ...
            foreach (var batch in batches)
            {
                ...
                if (totalTargetVertices > MaxTargetVerticesPerBake)
                {
                    error = $"the selected LODs contain {totalTargetVertices:N0} target vertices " +
                            $"(reached at LOD{batch.lodIndex}); the per-bake safety limit is " +
                            $"{MaxTargetVerticesPerBake:N0}. Exclude or simplify meshes before baking.";
                    return false;
                }
            }
